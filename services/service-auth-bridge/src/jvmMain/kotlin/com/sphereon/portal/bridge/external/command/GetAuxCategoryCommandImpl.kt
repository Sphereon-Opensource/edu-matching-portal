...
import com.sphereon.portal.bridge.external.GetAuxCategoryCommand
...
@ContributesBinding(SessionScope::class, binding = binding<GetAuxCategoryCommand>())
class GetAuxCategoryCommandImpl(
    execution: SessionExecution,
    private val authService: ExternalApiAuthService,
    private val auxiliaryDataService: AuxiliaryDataService,
) : HttpEndpointCommandAdapter(
    id = GetAuxCategoryCommand.COMMAND_ID,
    execution = execution,
    endpoint = GetAuxCategoryCommand.ENDPOINT,
    ...
    override suspend fun doExecute(
        ...
        val decrypted = auxiliaryDataService.getDecrypted(auth.tenantId, internalIdentityId, category)
        if (decrypted.isEmpty()) {
            return Ok(GenericHttpResponse(statusCode = 404,
                body = HttpJson.restApi.encodeToString(ExternalErrorResponse.serializer(),
                    ExternalErrorResponse("not_found", "Category not found")),
                headers = jsonHeaders))
        }

        val fields = decrypted.first().data
        val projected = authService.projectAuxiliaryFields(category, fields, auth.projection)

        return Ok(GenericHttpResponse(
            statusCode = 200,
            body = HttpJson.restApi.encodeToString(
                kotlinx.serialization.serializer<Map<String, JsonElement>>(), projected),
            headers = jsonHeaders,
        ))
    }
}
